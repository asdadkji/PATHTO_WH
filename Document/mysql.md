...
/*任务模块预添加数据*/
-- 为 task_templates 增加任务类型字段
ALTER TABLE `task_templates`
ADD COLUMN `task_type` ENUM('fixed_daily','self_selected','challenge') NOT NULL DEFAULT 'self_selected'
COMMENT '任务类型' AFTER `is_daily_repeat`;

-- 固定任务模板
INSERT INTO task_templates (title, description, default_points, category, is_daily_repeat, task_type, icon, color, is_active) VALUES
('完成家庭作业', '认真完成当天的家庭作业', 15, '学习', true, 'fixed_daily', 'EditPen', '#409EFF', true),
('阅读30分钟', '课外阅读至少30分钟', 10, '学习', true, 'fixed_daily', 'Reading', '#409EFF', true),
('整理房间', '整理自己的房间', 5, '家务', true, 'fixed_daily', 'HomeFilled', '#409EFF', true);

-- 自选任务模板
INSERT INTO task_templates (title, description, default_points, category, is_daily_repeat, task_type, icon, color, is_active) VALUES
('洗碗', '饭后洗碗', 5, '家务', false, 'self_selected', 'Bowl', '#67C23A', true),
('倒垃圾', '倒掉家里的垃圾', 3, '家务', false, 'self_selected', 'Delete', '#67C23A', true),
('做数学练习', '完成数学练习册', 10, '学习', false, 'self_selected', 'EditPen', '#67C23A', true),
('跳绳100次', '完成跳绳100次', 8, '运动', false, 'self_selected', 'Basketball', '#67C23A', true),
('早睡早起', '晚上9点前睡觉', 5, '习惯', false, 'self_selected', 'Moon', '#67C23A', true);

-- 挑战任务模板
INSERT INTO task_templates (title, description, default_points, category, is_daily_repeat, task_type, icon, color, is_active) VALUES
('完成一套试卷', '独立完成一套完整试卷', 50, '挑战', false, 'challenge', 'Trophy', '#E6A23C', true),
('帮助做一顿饭', '独立做一顿饭', 40, '挑战', false, 'challenge', 'Food', '#E6A23C', true),
('连续运动30分钟', '30分钟不间断运动', 35, '挑战', false, 'challenge', 'Basketball', '#E6A23C', true);

/*在 tasks 表增加 streak_count 字段*/
ALTER TABLE `tasks`
ADD COLUMN `streak_count` INT NOT NULL DEFAULT 0 COMMENT '该任务的连续打卡天数' AFTER `completed_at`;

/*为 task_templates 表新增连续打卡奖励字段*/
ALTER TABLE `task_templates`
ADD COLUMN `streak_bonus_7_days` INT NOT NULL DEFAULT 0 COMMENT '连续打卡7天奖励积分' AFTER `default_points`,
ADD COLUMN `streak_bonus_30_days` INT NOT NULL DEFAULT 0 COMMENT '连续打卡30天奖励积分' AFTER `streak_bonus_7_days`;