...
// 取消成功后调用 auth.refreshUserInfo() 从服务端同步最新积分并刷新列表。
// 当前积分取自 auth store 的 userInfo.totalPoints，页头展示。
import { computed, onMounted, ref } from 'vue'
import type { CSSProperties } from 'vue'
import { useRouter } from 'vue-router'
import { CircleClose, Goods, Refresh, Star, Tickets, Check } from '@element-plus/icons-vue'
import { cancelRedemption, getRedemptions, useRedemption } from '@/apis'
...
/** 立即使用已批准的兑换：approved → completed */
async function handleUse(item: RedemptionListItem) {
  try {
    await ElMessageBox.confirm(
      `确定已使用「${item.productName}」吗？`,
      '确认使用',
      { confirmButtonText: '已使用', cancelButtonText: '取消', type: 'info' },
    )
  } catch {
    return
  }
  usingId.value = item.redemptionId
  try {
    await useRedemption(item.redemptionId)
    ElMessage.success('已标记为使用完成')
    await loadRedemptions()
  } catch (e) {
    ElMessage.error(pickMsg(e, '操作失败，请稍后重试'))
  } finally {
    usingId.value = ''
  }
}
        ...
        >
          ...
          <div class="ri-actions">
            ...
            <el-button
              ...
              size="small"
              round
              :loading="usingId === item.redemptionId"
