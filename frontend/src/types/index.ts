...
/** 任务类型：固定每日 / 自选 / 挑战 */
export type TaskType = 'fixed_daily' | 'self_selected' | 'challenge'
...
/** 立即使用兑换响应 data（PUT /api/redemption/:id/use） */
...
/** 管理端任务模板条目（GET /api/admin/task-template/list） */
...
/** 创建模板入参（POST /api/admin/task-template） */
...
/** 更新模板入参（PUT /api/admin/task-template/:id，所有字段可选） */
...
/** 管理端商品条目（GET /api/admin/product/list） */
...
/** 更新商品入参（PUT /api/admin/product/:id，所有字段可选） */
...
/** 任务类型分布项 */
export interface TaskTypeDistItem {
  taskType: string
  count: number
}
...
/** 本月 vs 上月时间对比 */
export interface MonthlyTimeComparison {
  categories: string[]
